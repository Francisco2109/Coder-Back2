...
const login = async (req, res) => {
  ...
  const token = jwt.sign(userDto, config.JTW_SECTRET, { expiresIn: "1h" });
  res
    .cookie("coderCookie", token, { maxAge: 3600000 })
    .send({ status: "success", message: "Logged in", session: req.session });
};

const current = async (req, res) => {
  const cookie = req.cookies["coderCookie"];
  const user = jwt.verify(cookie, config.JTW_SECTRET);
...
const unprotectedLogin = async (req, res) => {
  ...
  if (!isValidPassword)
    return res
      .status(400)
      .send({ status: "error", error: "Incorrect password" });
  const token = jwt.sign(user, config.JTW_SECTRET, { expiresIn: "1h" });
  res
